...
A. Rectangular PDF
B. Gaussian PDF
C. It depends on whether the reading from the digital output is stable or not
...
A. Take the smallest scale of the measuring balance
B. Take the smallest scale of the measuring balance and divide by two
C. Calculate the standard deviation of the Gaussian PDF using (95% CI)/4
D. Calculate the standard deviation of the Rectangular PDF using half-width/sqrt(3)
...
If the reading from the digital output is stable, the uncertainty associated with it will be represented by the Rectangular PDF. However, if the reading is unstable, the uncertainty is dominated by the random uncertainty, in which case the standard uncertainty should be represented by the Gaussian PDF.

### Question 2

**A. Gaussian PDF**

As experimenters we need to make many small judgements in order to do this measurement (e.g., Where is the top of the spring? How does that line up on the ruler sitting a couple of cm away? How is is the ruler to read?). You can still determine a more likely answer, so the appropriate distribution is the peaked one, not a flat rectangular one. Therefore, the standard uncertainty of the measurement should be estimated using the Gaussian PDF.
